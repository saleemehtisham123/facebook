...
$get_posts = "SELECT * FROM posts WHERE user_id='$user_id' order by post_id DESC";
$run_get_posts = mysqli_query($con, $get_posts);
$output = "";
if (mysqli_num_rows($run_get_posts) > 0) {
    while ($row = mysqli_fetch_array($run_get_posts)) {
        $post_id = $row['post_id'];
        $like_info = get_like_status($post_id, $user_id);
        $total_likes = like_count($post_id);
        $comments = comments($post_id);
        $output = allPosts($row, $name, $surname, $like_info, $total_likes, $comments, $own_profile_img);
        echo $output;
    }
} else {
    $output .= "<div class='show-posts p-3 mt-4'><h1>hello</h1></div>";
}
function allPosts($row, $name, $surname, $like_info, $total_likes, $comments, $own_profile_img)
{
    $output = "";
    ...
    $output .= "<div class='post-top'><img src='profile_img/$own_profile_img' alt='error'>";

    $output .= "<b class='ml-2'>$name $surname</b><br><b class='ml-2'>$post_date</b><div class='post-edit'>...<div><p>Delete</p></div></div></div>";
